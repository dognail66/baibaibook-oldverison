...
import { buildBatchSummaryPrompt, buildBatchThinking, buildCharCardSystem, buildPersonaSystem, buildResummaryPrompt, buildSummaryPrompt, buildThinkingChecklist, buildThinkingPrefill, buildWorldInfoSystem, fmtItemLogInline, JAILBREAK_PROMPT, selectResolvedPlans } from './prompts';
...
function stripDisabledItemAnnotations(): void {
  if (!apiSettings.disabledFeatures.items) return;
  let changed = false;
  for (const message of getContext()?.chat ?? []) {
    const next = writeItemLogTag(message.mes, '');
    if (next === message.mes) continue;
    setMessageText(message, next);
    changed = true;
  }
  if (changed) scheduleLeafFlush();
}
...
export async function handleGenerationIntercept(
  ...
  if (apiSettings.allowGenerationWithPendingSummary) return false;
  ...
  if (opening >= 0) {
    ...
    if (inflight) {
      if (!apiSettings.silentBackfill) toast('正在为开场白建立时间锚点,请稍候…', 'info');
  ...
  if (holes.length === 1) {
    const inflight = currentSummaryPromise();
    if (inflight) {
      if (!apiSettings.silentBackfill) toast('正在补摘前一楼层,请稍候…', 'info');
      await inflight;
    } else if (!busy) {
      if (!apiSettings.silentBackfill) toast('正在补摘前一楼层,请稍候…', 'info');
...
async function summarizeFloorWork(
  ...
  const enabledPlans = stateBefore.plans.filter(p =>
    p.kind === 'plan' ? !apiSettings.disabledFeatures.plans : !apiSettings.disabledFeatures.suspenses);
  const openPlansOrdered = enabledPlans.filter(p => p.status === 'open');
  const prompt = buildSummaryPrompt({
    ...
    // 全部已了结计划:与面板、主模型注入同口径;截止点用本楼之前的状态(不泄漏未来)
    resolvedPlans: selectResolvedPlans(enabledPlans),
    history,
    content,
    hasTimeTags,
  ...
  messages.push(
    ...
    { role: 'system', content: buildThinkingChecklist() },
    { role: 'assistant', content: buildThinkingPrefill() },
  );
...
export function bindEngine(): void {
  ...
  // 单项功能关闭后立即重建状态槽，保证刚勾选就不再把对应数据发给主模型。
  watch(
    () => [
      apiSettings.disabledFeatures.items,
      apiSettings.disabledFeatures.scenes,
