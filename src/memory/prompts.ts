...
export const SUMMARY_MACROS: PromptMacro[] = [
  ...
  { token: '{{resolved_plans_block}}', desc: '全部已了结的计划/悬念(含了结方式/原因,防重复记录)' },
...
summary 用于记录本回合发生的剧情;除此之外启用的结构化字段属于长期状态数据库。
...
【严格边界】
  ...
  ✦ 其它信息写入对应的已启用字段,不要在 protagonist 里重复。
...
【主要角色的离场演变(唯一允许的合理推演,务必克制)】
  ...
  ✦ 严格边界:此推演**仅限主要角色、仅限 outfit/location/condition 三个覆盖型字段**;**绝不可**外溢到 summary 正文或其它结构化字段,也不可用于普通配角——那些仍严格只记正文明写,禁止脑补。推演要符合常理、点到为止,不要编造具体剧情事件。
...
━━ 新增前查重:按未知答案/待决事件归一 ━━
写任何 plans.add 前,逐条核对上方【未了结】与【已了结】条目:
...
★ 了结必写:本楼若有事件使先前提出的要求、意图、义务或威胁**被作废、取消、化解或了结**,summary 必须显式写出这个**结果**("……故此事作废/无需再去/已化解"),不能只叙述冲突过程而漏掉「所以不用做了」这一结论。
...
{{location_path_field}}
  ...
  "npcs": {
    ...
    "remove": ["永久退场的已有NPC名"]
  }{{items_field}}{{scenes_field}}{{plans_field}}{{vars_field}}
}
...
- 每个元素只含 n / summary / timeStart / timeEnd,不要输出其它结构化字段。
- 严禁输出 JSON 以外的任何内容(不要解释、不要思维链、不要代码块围栏)。`;
...
4. 只产摘要+时间:每个元素只含 n / summary / timeStart / timeEnd,不输出其它结构化字段。
...
interface BuildArgs {
  ...
  /** 全部已完成的计划/悬念(按 resolvedAt 倒序);防副模型重复记录。空数组→渲染「(无)」 */
...
/** 选出全部已了结计划/悬念，按了结时间倒序。面板、主模型注入与副 API 共用，禁止形成隐藏队列。 */
export function selectResolvedPlans(plans: MemPlan[]): MemPlan[] {
  const byTimeDesc = (a: MemPlan, b: MemPlan): number => (b.resolvedAt ?? 0) - (a.resolvedAt ?? 0);
  return plans.filter(p => p.status === 'resolved').sort(byTimeDesc);
}
...
/**
 * 渲染「已了结的计划/悬念」列表(不编号——它们已了结,无需像未了结项那样供 resolve 指代)。
...
const SCENES_FIELD_TMPL = `,
  "scenes": {
    "add": [{ "path": ["上级区域","具体地点"], "desc": "地点描述(必填,简短客观)" }],
    "update": [{ "path": ["已有地点的完整路径"], "desc": "更新后的完整累积描述" }],
    "reparent": [{ "node": ["已有地点当前完整路径"], "newPath": ["新上级","...","该地点"], "descs": { "新上级": "新上级的描述" } }]
  }`;

const RULE_PLAN_ONLY = `═══ 【计划规则】(plans 字段) ═══
计划默认不记。只有需要跨越当前场景、正文明确给出行动主体真心决定或承诺的未来行动才记录；客套、敷衍、口嗨、临时动作一律只写 summary。
新增前对照未了结与全部已了结计划查重，同一行动不得换说法重复 add。
新增格式:{ "kind":"plan", "content":"主体明确要做的未来行动", "createdTime":"立下时的故事内时间", "targetTime":"明确或模糊的目标时间(可选)" }。
正文明确说明计划已达成、取消或失败时 resolve，格式:{ "id":"p1", "outcome":"done|cancelled|failed", "reason":"关键事实+结果" }；不确定则保留。`;

const RULE_SUSPENSE_ONLY = `═══ 【悬念规则】(plans 字段) ═══
悬念默认不记，只允许两类:正文明确留下、且有线索支撑的待揭晓事实；已经实际启动、正在等待客观结果的外部事件。
已解释的设定、持续状态、人物两难、关系张力、潜在危险，以及用“会不会/将如何”包装出来的问题都不是悬念，只写 summary。
新增前按未知答案或待决事件对照未了结与全部已了结悬念查重；同一答案或同一事件不得重复 add。
新增格式:{ "kind":"suspense", "content":"已知关键事实+精确未决点", "createdTime":"出现时的故事内时间" }。
正文明确揭晓、作废或落空时 resolve，格式:{ "id":"p1", "outcome":"done|cancelled|failed", "reason":"关键事实+结果" }；不确定则保留。`;

function plansRule(): string {
  const { plans, suspenses } = apiSettings.disabledFeatures;
  if (!plans && !suspenses) return RULE_PLANS;
  if (!plans) return RULE_PLAN_ONLY;
  if (!suspenses) return RULE_SUSPENSE_ONLY;
  return '';
}

function plansField(): string {
  const add: string[] = [];
  if (!apiSettings.disabledFeatures.plans) {
    add.push('{ "kind": "plan", "content": "新出现的计划/目标", "createdTime": "立计划时的故事内时间", "targetTime": "目标时间(可选)" }');
  }
  if (!apiSettings.disabledFeatures.suspenses) {
    add.push('{ "kind": "suspense", "content": "待揭晓事实或已启动外部事件", "createdTime": "悬念出现时的故事内时间" }');
  }
  if (!add.length) return '';
  return `,
  "plans": {
    "add": [${add.join(', ')}],
    "resolve": [{ "id": "p1", "outcome": "done|cancelled|failed", "reason": "一句话说明如何了结" }]
  }`;
}

function plansStateSection(a: BuildArgs): string {
  if (apiSettings.disabledFeatures.plans && apiSettings.disabledFeatures.suspenses) return '';
  const enabled = (kind: 'plan' | 'suspense') =>
    kind === 'plan' ? !apiSettings.disabledFeatures.plans : !apiSettings.disabledFeatures.suspenses;
  const open = a.openPlans.filter(p => enabled(p.kind));
  const resolved = a.resolvedPlans.filter(p => enabled(p.kind));
  const label = apiSettings.disabledFeatures.plans ? '悬念' : apiSettings.disabledFeatures.suspenses ? '计划' : '计划/悬念';
  return `- 未了结的${label}(用编号 p1、p2… 指代):
${fmtPlans(open)}
- 已了结的${label}(只读参考;禁止重复新增或再次 resolve):
${fmtResolvedPlans(resolved)}`;
}
...
export function buildSummaryPrompt(a: BuildArgs): string {
  ...
  const macros = {
    ...
    items_block: apiSettings.disabledFeatures.items ? '' : fmtItems(a.items),
    itemlog_block: apiSettings.disabledFeatures.items ? '' : fmtItemLog(a.itemLog),
    scenes_block: apiSettings.disabledFeatures.scenes ? '' : fmtScenes(a.scenes),
    npcs_block: fmtNpcs(a.npcs),
    plans_block: apiSettings.disabledFeatures.plans && apiSettings.disabledFeatures.suspenses ? '' : fmtPlans(a.openPlans),
    resolved_plans_block: apiSettings.disabledFeatures.plans && apiSettings.disabledFeatures.suspenses ? '' : fmtResolvedPlans(a.resolvedPlans),
    items_state_section: apiSettings.disabledFeatures.items
      ? ''
      : `- 现有物品:\n${fmtItems(a.items)}\n- 近期物品变动(已结算,严禁重复结算):\n${fmtItemLog(a.itemLog)}`,
    scenes_state_section: apiSettings.disabledFeatures.scenes
      ? ''
      : `- 已知地点(复用命名、勿重复记录):\n${fmtScenes(a.scenes)}`,
    plans_state_section: plansStateSection(a),
    location_path_field: apiSettings.disabledFeatures.scenes
      ? ''
      : '  "locationPath": ["与 location 对应的已知场景节点完整路径,由粗到细"],',
    items_field: apiSettings.disabledFeatures.items ? '' : ITEMS_FIELD_TMPL,
    scenes_field: apiSettings.disabledFeatures.scenes ? '' : SCENES_FIELD_TMPL,
    plans_field: plansField(),
    items_rule: apiSettings.disabledFeatures.items ? '' : RULE_ITEMS,
    scenes_rule: apiSettings.disabledFeatures.scenes ? '' : RULE_SCENES,
    plans_rule: plansRule(),
    content: a.content,
...
2c. NPC 盘点(对照【已登场NPC】名册)—— 门槛是用来挡路人的,不是让你偷懒
   ...
   - 离场演变(仅限★主要角色):名册里某★主要角色与主角已分开**明显跨越时间**(数日/长旅程)后又出现或被提及?→ 可合理推演并 update 其 outfit/location/condition(多日多半已换装/已移动/伤已变化),避免「重逢还穿老样子」。**仅限主要角色这三个字段,不可外溢到正文、其它字段或配角。**
...
3. 悬念簿清算(分两步,先计划后悬念)
   ...
   - 查重:将每条候选归一为【未知答案】或【待决外部事件】,逐条对照未了结和全部已了结条目。答案相同/同一事件 → 不 add;新线索、阶段进展、规则细节、人物反应只写 summary。根源相同但答案确实不同、可独立揭晓时才允许分开。
...
export function buildThinkingChecklist(): string {
  let checklist = THINKING_CHECKLIST;
  ...
  if (apiSettings.disabledFeatures.scenes) {
    checklist = checklist
      ...
      .replace(
        '   - 时间、地点相比【当前已知状态】有无变化?地点变了→写 location,并同步给 locationPath(对应【已知地点】树里那个节点的完整路径,对不到细节就给到能对上的上级)。',
        '   - 时间、地点相比【当前已知状态】有无变化?地点变了→写 location。',
      );
  }
  if (apiSettings.disabledFeatures.plans || apiSettings.disabledFeatures.suspenses) {
    ...
    if (!apiSettings.disabledFeatures.plans) {
      replacement = `\n3. 计划清算
   - 对照未完成计划逐条判断是否已达成、取消或失败；明确了结才 resolve，不确定则保留。
   - 只新增正文明确、真心决定且会跨越当前场景的未来行动；客套、敷衍和临时动作只写 summary。`;
    } else if (!apiSettings.disabledFeatures.suspenses) {
      replacement = `\n3. 悬念清算
   - 对照未解事项逐条判断答案是否已揭晓、作废或落空；明确了结才 resolve，不确定则保留。
   - 只新增正文明确提出的待揭晓事实或已启动外部事件；持续状态、人物两难与潜在危险只写 summary。`;
    }
    checklist = checklist.replace(/\n3\. 悬念簿清算[\s\S]*?(?=\n4\. summary 收笔位置确认)/, replacement);
  }
  return checklist;
}

export function buildThinkingPrefill(): string {
  const fields = ['summary', 'time', 'location', 'protagonist', 'npcs'];
  if (!apiSettings.disabledFeatures.items) fields.push('items');
  if (!apiSettings.disabledFeatures.scenes) fields.push('scenes');
  if (!apiSettings.disabledFeatures.plans || !apiSettings.disabledFeatures.suspenses) fields.push('plans');
  return `<thinking>
收到,我先按检查点逐条梳理,然后只输出一个 JSON 对象(字段 ${fields.join('/')}),
不输出 markdown 围栏、不在思考与 JSON 之间插入解释。
