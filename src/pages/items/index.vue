...
function removeItem(id: string) {
  ...
  if (!it) return;
  if (appendOpToLatestLeaf({ items: { remove: [it.name] } })) refreshInjection();
}

/* —— 长按编辑键进入多选,与计划/悬念页面保持同一交互 —— */
const itemSelectMode = ref(false);
const selectedItemIds = ref<Set<string>>(new Set());
const selectedItemCount = computed(() => memory.items.filter(i => selectedItemIds.value.has(i.id)).length);
const allItemsSelected = computed(() => memory.items.length > 0 && memory.items.every(i => selectedItemIds.value.has(i.id)));
const itemDeleteConfirmOpen = ref(false);
let itemLongPressTimer: ReturnType<typeof setTimeout> | null = null;
let itemLongPressActivated = false;
let ignoreNextItemClick = false;
const ITEM_LONG_PRESS_MS = 550;

function clearItemLongPressTimer() {
  if (itemLongPressTimer !== null) {
    clearTimeout(itemLongPressTimer);
    itemLongPressTimer = null;
  }
}
function enterItemSelectMode(initialId: string) {
  itemSelectMode.value = true;
  selectedItemIds.value = new Set([initialId]);
}
function exitItemSelectMode() {
  clearItemLongPressTimer();
  itemLongPressActivated = false;
  ignoreNextItemClick = false;
  itemDeleteConfirmOpen.value = false;
  itemSelectMode.value = false;
  selectedItemIds.value = new Set();
}
function startItemLongPress(event: PointerEvent, id: string) {
  if (itemSelectMode.value || event.button !== 0) return;
  clearItemLongPressTimer();
  itemLongPressActivated = false;
  ignoreNextItemClick = false;
  itemLongPressTimer = setTimeout(() => {
    itemLongPressTimer = null;
    itemLongPressActivated = true;
    ignoreNextItemClick = true;
    enterItemSelectMode(id);
  }, ITEM_LONG_PRESS_MS);
}
function finishItemLongPress() {
  clearItemLongPressTimer();
  if (!itemLongPressActivated) return;
  itemLongPressActivated = false;
  setTimeout(() => { ignoreNextItemClick = false; }, 0);
}
function cancelItemLongPress() {
  clearItemLongPressTimer();
  if (itemLongPressActivated) setTimeout(() => { ignoreNextItemClick = false; }, 0);
  itemLongPressActivated = false;
}
let offItemChatChanged: (() => void) | null = null;
onMounted(() => {
  const ctx = getContext();
  if (ctx?.eventSource && ctx.eventTypes?.CHAT_CHANGED) {
    ctx.eventSource.on(ctx.eventTypes.CHAT_CHANGED, exitItemSelectMode);
    offItemChatChanged = () => ctx.eventSource.off?.(ctx.eventTypes.CHAT_CHANGED, exitItemSelectMode);
  }
});
onUnmounted(() => {
  cancelItemLongPress();
...
function handleItemCardClick(id: string) {
  if (!itemSelectMode.value) return;
...
function toggleAllItems() {
  selectedItemIds.value = allItemsSelected.value ? new Set() : new Set(memory.items.map(i => i.id));
}
function openItemDeleteConfirm() {
  if (selectedItemCount.value) itemDeleteConfirmOpen.value = true;
}
function deleteSelectedItems() {
  itemDeleteConfirmOpen.value = false;
  const names = memory.items.filter(i => selectedItemIds.value.has(i.id)).map(i => i.name);
  if (!names.length) return;
  if (!appendOpToLatestLeaf({ items: { remove: names } })) {
    toast('删除失败：需要先有摘要才能写入', 'warning');
    return;
  }
  refreshInjection();
  exitItemSelectMode();
  toast(`已删除 ${names.length} 件物品`, 'success');
}
...
<template>
  <section class="bbs-page">
    ...
    <div v-if="itemSelectMode && memory.items.length" class="bbs-item-select-bar">
      <span class="bbs-item-select-info">
        {{ selectedItemCount ? `已选 ${selectedItemCount} 件物品` : '请选择要删除的物品' }}
      </span>
      <button class="bbs-btn bbs-btn-sm" type="button" @click="toggleAllItems">
        {{ allItemsSelected ? '取消全选' : '全选' }}
      </button>
      <button class="bbs-btn bbs-btn-sm bbs-btn-danger" type="button" :disabled="!selectedItemCount" @click="openItemDeleteConfirm">
        <Icon name="trash" />删除所选
      </button>
      <button class="bbs-btn bbs-btn-sm" type="button" @click="exitItemSelectMode">
        <Icon name="close" />退出
      </button>
    </div>
    ...
    <div v-if="memory.items.length" class="bbs-item-list">
      <div
        v-for="it in memory.items"
        :key="it.id"
        class="bbs-item"
        :class="{ 'is-selecting': itemSelectMode, 'is-selected': itemSelectMode && selectedItemIds.has(it.id) }"
        :role="itemSelectMode ? 'checkbox' : undefined"
        :aria-checked="itemSelectMode ? selectedItemIds.has(it.id) : undefined"
        :tabindex="itemSelectMode ? 0 : undefined"
        @click="handleItemCardClick(it.id)"
        @keydown.enter.prevent="itemSelectMode && toggleItemSelect(it.id)"
        @keydown.space.prevent="itemSelectMode && toggleItemSelect(it.id)"
      >
        <div class="bbs-item-head">
          <span v-if="itemSelectMode" class="bbs-item-check" aria-hidden="true">
            <input class="bbs-checkbox" type="checkbox" :checked="selectedItemIds.has(it.id)" tabindex="-1" @click.stop="toggleItemSelect(it.id)" />
          </span>
          ...
          <span v-if="!itemSelectMode" class="bbs-item-acts">
            <button
              class="bbs-item-act"
              type="button"
              title="编辑；长按进入多选"
              @pointerdown="startItemLongPress($event, it.id)"
              @pointerup="finishItemLongPress"
              @pointercancel="cancelItemLongPress"
              @pointerleave="cancelItemLongPress"
              @contextmenu.prevent
              @click.stop="openEdit(it.id)"
            >
              <Icon name="edit" />
            </button>
    ...
    <ConfirmDialog
      v-model:open="itemDeleteConfirmOpen"
      title="删除所选物品"
      confirm-text="删除"
      confirm-icon="trash"
      tone="danger"
    ...
    >
      将永久删除选中的 {{ selectedItemCount }} 件物品，并立即从后台提示词中移除。继续？
    </ConfirmDialog>
...
.bbs-item.is-selecting {
  cursor: pointer;
}
.bbs-item.is-selected {
...
.bbs-item-select-info {
  flex: 1 1 auto;
  min-width: 0;
  color: var(--bbs-ink-soft);
}
.bbs-item-select-bar .bbs-btn-danger {
