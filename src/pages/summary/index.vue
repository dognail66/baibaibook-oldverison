...
import { outcomeLabel } from '@/memory/prompts';
...
const resetViewStates = () => {
  ...
  exitSelectMode();
  exitPlanSelectMode();
  planViewMode.value = 'recent';
  suspenseViewMode.value = 'recent';
};
...
// 后台仍保留全部计划/悬念；面板默认只渲染全部进行中 + 最近 5 条已结案，避免大量历史卡片拖慢首屏。
// 用户切到「全部」即可查看和删除完整集合。
const panelPlans = computed(() => {
  const enabled = (kind: 'plan' | 'suspense') =>
    kind === 'plan' ? !apiSettings.disabledFeatures.plans : !apiSettings.disabledFeatures.suspenses;
  return memory.plans.filter(p => enabled(p.kind));
});
type PlanViewMode = 'recent' | 'all';
const RECENT_RESOLVED_COUNT = 5;
const planViewMode = ref<PlanViewMode>('recent');
const suspenseViewMode = ref<PlanViewMode>('recent');
const allPlansOnly = computed(() => panelPlans.value.filter(p => p.kind !== 'suspense'));
const allSuspenses = computed(() => panelPlans.value.filter(p => p.kind === 'suspense'));
function recentPlans(items: MemPlan[]): MemPlan[] {
...
const plansOnly = computed(() => planViewMode.value === 'all' ? allPlansOnly.value : recentPlans(allPlansOnly.value));
const suspenses = computed(() => suspenseViewMode.value === 'all' ? allSuspenses.value : recentPlans(allSuspenses.value));
const visiblePanelPlans = computed(() => [...plansOnly.value, ...suspenses.value]);
const planSelectMode = ref(false);
const selectedPlanIds = ref<Set<string>>(new Set());
const selectedPlanCount = computed(() => visiblePanelPlans.value.filter(p => selectedPlanIds.value.has(p.id)).length);
const allPanelPlansSelected = computed(
  () => visiblePanelPlans.value.length > 0 && visiblePanelPlans.value.every(p => selectedPlanIds.value.has(p.id)),
);
let planLongPressTimer: ReturnType<typeof setTimeout> | null = null;
let planLongPressActivated = false;
let ignoreNextPlanClick = false;
const PLAN_LONG_PRESS_MS = 550;

function clearPlanLongPressTimer() {
  if (planLongPressTimer !== null) {
    clearTimeout(planLongPressTimer);
    planLongPressTimer = null;
  }
}
function enterPlanSelectMode(initialId: string) {
  exitSelectMode();
  planSelectMode.value = true;
  selectedPlanIds.value = new Set([initialId]);
}
function exitPlanSelectMode() {
  clearPlanLongPressTimer();
  planLongPressActivated = false;
  ignoreNextPlanClick = false;
  planDeleteConfirmOpen.value = false;
  planSelectMode.value = false;
  selectedPlanIds.value = new Set();
}
function startPlanLongPress(event: PointerEvent, id: string) {
  if (planSelectMode.value || event.button !== 0) return;
  clearPlanLongPressTimer();
  planLongPressActivated = false;
  ignoreNextPlanClick = false;
  planLongPressTimer = setTimeout(() => {
    planLongPressTimer = null;
    planLongPressActivated = true;
    ignoreNextPlanClick = true;
    enterPlanSelectMode(id);
  }, PLAN_LONG_PRESS_MS);
}
function finishPlanLongPress() {
  clearPlanLongPressTimer();
  if (!planLongPressActivated) return;
  planLongPressActivated = false;
  // pointerup 后浏览器可能补发 click；等它结束再解除拦截，避免刚进入多选就把首项取消。
  setTimeout(() => { ignoreNextPlanClick = false; }, 0);
}
function cancelPlanLongPress() {
  clearPlanLongPressTimer();
  if (planLongPressActivated) setTimeout(() => { ignoreNextPlanClick = false; }, 0);
  planLongPressActivated = false;
}
function togglePlanSelect(id: string) {
  const next = new Set(selectedPlanIds.value);
  if (next.has(id)) next.delete(id);
  else next.add(id);
  selectedPlanIds.value = next;
}
function handlePlanCardClick(id: string) {
  if (!planSelectMode.value) return;
  if (ignoreNextPlanClick) {
    ignoreNextPlanClick = false;
    return;
  }
  togglePlanSelect(id);
}
function toggleAllPanelPlans() {
  selectedPlanIds.value = allPanelPlansSelected.value
    ? new Set()
    : new Set(visiblePanelPlans.value.map(p => p.id));
}
const resolvedPanelPlanCount = computed(() => visiblePanelPlans.value.filter(p => p.status === 'resolved').length);
function selectResolvedPanelPlans() {
  // 选择范围直接替换为当前视图中的已结案条目；要清理全部历史时先切换为「全部」。
  selectedPlanIds.value = new Set(
    visiblePanelPlans.value.filter(p => p.status === 'resolved').map(p => p.id),
  );
}
function setPlanViewMode(kind: 'plan' | 'suspense', mode: PlanViewMode) {
  if (kind === 'plan') planViewMode.value = mode;
  else suspenseViewMode.value = mode;
  // 从「全部」切回「近期」时清掉已隐藏条目的勾选，避免不可见删除。
  const visibleIds = new Set(visiblePanelPlans.value.map(p => p.id));
  selectedPlanIds.value = new Set([...selectedPlanIds.value].filter(id => visibleIds.has(id)));
}
...
function removePlan(id: string) {
  if (appendOpToLatestLeaf({ plans: { remove: [id] } })) refreshInjection();
}
...
function handlePlanEditClick(p: Parameters<typeof openPlanEdit>[0]) {
  if (ignoreNextPlanClick) {
    ignoreNextPlanClick = false;
    return;
  }
...
const planDeleteConfirmOpen = ref(false);
function openPlanDeleteConfirm() {
  if (selectedPlanCount.value) planDeleteConfirmOpen.value = true;
}
function runDeleteSelectedPlans() {
  planDeleteConfirmOpen.value = false;
  const ids = panelPlans.value.filter(p => selectedPlanIds.value.has(p.id)).map(p => p.id);
  if (!ids.length) return;
  if (!appendOpToLatestLeaf({ plans: { remove: ids } })) {
    toast('删除失败：需要先有摘要才能写入', 'warning');
    return;
  }
  refreshInjection();
  exitPlanSelectMode();
  toast(`已删除 ${ids.length} 条计划/悬念`, 'success');
}
...
<template>
  <section class="bbs-page">
    <!-- ===== 计划 / 悬念:顶部两区,各自折叠计数 ===== -->
    <!-- 长按编辑键进入的计划/悬念多选操作条。置顶吸附，长列表中也能随时删除或退出。 -->
    <div v-if="planSelectMode && panelPlans.length" class="bbs-select-bar bbs-plan-select-bar">
      <span class="bbs-select-info">
        {{ selectedPlanCount ? `已选 ${selectedPlanCount} 条计划/悬念` : '请选择要删除的计划/悬念' }}
      </span>
      <button
        class="bbs-btn bbs-btn-sm"
        type="button"
        :title="allPanelPlansSelected ? '取消全选' : '全选当前面板中的计划和悬念'"
        @click="toggleAllPanelPlans"
      >
        {{ allPanelPlansSelected ? '取消全选' : '全选' }}
      </button>
      <button
        class="bbs-btn bbs-btn-sm"
        type="button"
        :disabled="!resolvedPanelPlanCount"
        :title="resolvedPanelPlanCount ? `只勾选全部 ${resolvedPanelPlanCount} 条已结案内容` : '没有已结案内容'"
        @click="selectResolvedPanelPlans"
      >
        全选已过期
      </button>
      <button
        ...
        type="button"
        :disabled="!selectedPlanCount"
        @click="openPlanDeleteConfirm"
      >
        <Icon name="trash" />删除所选
      </button>
      <button class="bbs-btn bbs-btn-sm" type="button" title="退出多选" @click="exitPlanSelectMode">
        <Icon name="close" />退出
      </button>
    </div>
    ...
    <div v-for="g in foldGroups" :key="g.kind" class="bbs-fold-section">
      <div class="bbs-section-head">
        ...
        >
          ...
          <span v-if="g.foldable" class="bbs-fold-count">
            {{ g.mode === 'recent' && g.items.length < g.total ? `${g.items.length}/${g.total}` : g.total }}
          </span>
        </button>
        <div v-if="g.total" class="bbs-plan-view-toggle" role="group" :aria-label="`${g.title}显示范围`">
          <button
            class="bbs-plan-view-option"
            :class="{ 'is-active': g.mode === 'recent' }"
            type="button"
            title="显示全部进行中及最近 5 条已结案内容；后台提示词仍保留全部"
            @click="setPlanViewMode(g.kind, 'recent')"
          >近期</button>
          <button
            class="bbs-plan-view-option"
            :class="{ 'is-active': g.mode === 'all' }"
            type="button"
            title="显示全部内容，便于检查和批量删除"
            @click="setPlanViewMode(g.kind, 'all')"
          >全部</button>
        </div>
        <button
          class="bbs-add-mini"
      ...
      <div class="bbs-fold-wrap" :class="{ 'is-collapsed': !g.shown }">
        <div class="bbs-fold-inner">
          <div v-if="g.items.length" class="bbs-plan-group" :class="{ 'is-selecting': planSelectMode }">
            <div
              v-for="p in g.items"
              :key="p.id"
              class="bbs-plan"
              :class="{ 'is-selected': planSelectMode && selectedPlanIds.has(p.id) }"
              :role="planSelectMode ? 'checkbox' : undefined"
              :aria-checked="planSelectMode ? selectedPlanIds.has(p.id) : undefined"
              :tabindex="planSelectMode ? 0 : undefined"
              @click="handlePlanCardClick(p.id)"
              @keydown.enter.prevent="planSelectMode && togglePlanSelect(p.id)"
              @keydown.space.prevent="planSelectMode && togglePlanSelect(p.id)"
            >
              <div class="bbs-plan-head">
                <span v-if="planSelectMode" class="bbs-plan-check" aria-hidden="true">
                  <input
                    class="bbs-checkbox"
                    type="checkbox"
                ...
                <span v-if="p.status === 'resolved'" class="bbs-plan-injected">
                  {{ outcomeLabel(p.kind, p.outcome) }} · 仍在提示词
                </span>
                <span v-if="planFloor(p.id) !== undefined" class="bbs-plan-floor">#{{ planFloor(p.id) }}</span>
                <span v-if="!planSelectMode" class="bbs-plan-acts">
                  <button
                    class="bbs-plan-act bbs-plan-edit"
                    type="button"
                    title="编辑；长按进入多选"
                    @pointerdown="startPlanLongPress($event, p.id)"
                    @pointerup="finishPlanLongPress"
                    @pointercancel="cancelPlanLongPress"
                    @pointerleave="cancelPlanLongPress"
                    @contextmenu.prevent
                    @click.stop="handlePlanEditClick(p)"
              ...
              <p v-if="p.status === 'resolved' && p.resolvedReason" class="bbs-plan-resolved-reason">
                结案：{{ p.resolvedReason }}
              </p>
    ...
    <ConfirmDialog
      v-model:open="planDeleteConfirmOpen"
      title="删除所选计划/悬念"
      confirmText="删除"
      confirmIcon="trash"
      tone="danger"
      @confirm="runDeleteSelectedPlans"
    >
      将永久删除选中的 {{ selectedPlanCount }} 条计划/悬念，并立即从后台提示词中移除。继续？
    </ConfirmDialog>

    <!-- 分章分隔:两侧细线 + 居中金色菱形(古籍分章鱼尾标记),比普通 hr 更明确地隔开两区 -->
    <div v-if="foldGroups.length" class="bbs-divider" role="separator" aria-hidden="true">
...
.bbs-plan-view-option.is-active {
  background: var(--bbs-surface);
  color: var(--bbs-accent);
  box-shadow: 0 0 0 1px var(--bbs-line);
}
.bbs-plan-view-option:focus-visible {
...
.bbs-plan-group.is-selecting .bbs-plan {
  cursor: pointer;
  outline: none;
  transition: border-color 0.15s, background 0.15s;
}
.bbs-plan-group.is-selecting .bbs-plan:hover:not(.is-selected) {
  border-color: var(--bbs-line-strong);
}
.bbs-plan-group.is-selecting .bbs-plan:focus-visible {
  outline: 2px solid var(--bbs-accent);
  outline-offset: 1px;
}
.bbs-plan.is-selected {
...
@media (max-width: 640px) {
  ...
  /* 计划多选操作条：提示独占首行，四个动作在窄屏分成两行，避免长文案挤压。 */
  .bbs-plan-select-bar .bbs-select-info {
    flex: 1 0 100%;
  }
  .bbs-plan-select-bar .bbs-btn {
