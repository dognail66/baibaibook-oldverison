...
/** 单项结构化记忆关闭开关。关闭只停用采集、注入与面板入口，既有数据保留供以后重新启用。 */
...
export interface ApiSettings {
  ...
  /** 允许上一楼尚未摘要时继续生成。默认开启；关闭后恢复旧版积压拦截。 */
  allowGenerationWithPendingSummary: boolean;
  /** 自动补摘在后台静默运行。开启只隐藏等待通知，不影响摘要分析与写入。 */
...
function defaults(): ApiSettings {
  return {
    ...
    allowGenerationWithPendingSummary: true,
    silentBackfill: false,
    summaryOnlyMode: false,
    disabledFeatures: { items: false, scenes: false, plans: false, suspenses: false },
    keepRecent: 3,
    excludedChars: [],
    excludedWorldNames: [],
...
function normalize(raw: unknown): ApiSettings {
  ...
  const disabled = ((raw as Partial<ApiSettings>).disabledFeatures ?? {}) as Partial<DisabledFeatures>;
  merged.disabledFeatures = {
    items: typeof disabled.items === 'boolean' ? disabled.items : false,
    scenes: typeof disabled.scenes === 'boolean' ? disabled.scenes : false,
    plans: typeof disabled.plans === 'boolean' ? disabled.plans : false,
    suspenses: typeof disabled.suspenses === 'boolean' ? disabled.suspenses : false,
  };
  ...
  merged.allowGenerationWithPendingSummary =
    typeof merged.allowGenerationWithPendingSummary === 'boolean'
      ? merged.allowGenerationWithPendingSummary
      : true;
  merged.silentBackfill = typeof merged.silentBackfill === 'boolean' ? merged.silentBackfill : false;
...
function applyInto(target: ApiSettings, src: ApiSettings): void {
  ...
  target.allowGenerationWithPendingSummary = src.allowGenerationWithPendingSummary;
  target.silentBackfill = src.silentBackfill;
  target.summaryOnlyMode = src.summaryOnlyMode;
  target.disabledFeatures = src.disabledFeatures;
